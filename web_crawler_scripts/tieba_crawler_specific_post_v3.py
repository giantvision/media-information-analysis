...
class TiebaPostScraper:
    ...
    def _parse_client_posts(self, post_list: list, full_data: dict) -> list:
        ...
        # 【修复 #2】构建 user_map：客户端 API 的完整用户信息在顶层 user_list 中，
        # 每个 post 的 author 可能只有 id，其他字段为空
        ...
        for post in post_list:
            ...
            # 【修复 #2】用户信息：先从 post.author 取，若为空则从 user_map 补全
            uid = str(author_raw.get('id', ''))
            user_name = author_raw.get('name', '')
            nick_name = author_raw.get('name_show', '') or user_name
            level_id = safe_int(author_raw.get('level_id', 0))

            # 如果 author 中 name 为空，从 user_map 补全
            if uid and (not user_name or not nick_name) and uid in user_map:
                u_info = user_map[uid]
                user_name = user_name or u_info.get('user_name', '')
                nick_name = nick_name or u_info.get('nick_name', '') or user_name
                level_id = level_id or u_info.get('level_id', 0)
            ...
            # 【修复 #1】第1楼是主帖：存到 result 元数据，但不加入 comments 列表
            if floor_num == 1:
            ...
            if isinstance(sub_post_list, dict):
                ...
                if sub_posts:
                    for sp in sub_posts:
                        ...
                        sp_name = sp_author.get('name_show', '') or sp_author.get('name', '')
                        # 同样从 user_map 补全
                        if sp_uid and not sp_name and sp_uid in user_map:
